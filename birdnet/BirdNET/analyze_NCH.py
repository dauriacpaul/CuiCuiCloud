...
from birdnet.BirdNET import config as cfg
from birdnet.BirdNET import audio
from birdnet.BirdNET import model
...
def writeErrorLog(msg):
    with open(cfg.ERROR_LOG_FILE, 'a', encoding="utf8") as elog:
        elog.write(msg + '\n')
    ...
    try:
        ...
        for c in range(len(chunks)):
            ...
            for i in range(len(samples)):
                ...
                # Assign scores to labels
                p_labels = dict(zip(cfg.LABELS, pred))
                print('Labels list contains {} species'.format(len(cfg.LABELS)))
                print('Species list contains {} species'.format(len(cfg.SPECIES_LIST)))
                # Sort by score
                p_sorted = sorted(p_labels.items(), key=operator.itemgetter(1), reverse=True)
                # Extract valid predictions for every timestamp
                p_sorted = [c for c in p_sorted if c[1] > cfg.MIN_CONFIDENCE and c[0] in cfg.CODES and (
                            c[0] in cfg.SPECIES_LIST or len(cfg.SPECIES_LIST) == 0)]
                # Store top results and advance indices
                # if p_sorted:
                # results[(s_start, s_end)] = p_sorted
                for c in p_sorted:
                    results.append([s_start, s_end, c[0].split('_')[0], c[0].split('_')[1], c[1]])
